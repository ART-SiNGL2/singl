...
import singl.util.LabelMatchType;
import singl.util.PipeUtil;

public class Labeller extends SiNGLBase {

	protected static String name = "singl.Labeller";
	public static final Log LOG = LogFactory.getLog(Labeller.class);
	long positivePairsCount, negativePairsCount, notSurePairsCount;
	long totalCount;

	public Labeller() {
		setSiNGLOptions(SiNGLOptions.LABEL);
	}
	...
	public Dataset<Row> getUnmarkedRecords() throws SiNGLClientException {
	...
	public void processRecordsCli(Dataset<Row> lines) throws SiNGLClientException {
		...
		try {
			...
			for (int index = 0; index < totalPairs; index++){	
				...
				if (prediction == ColValues.IS_NOT_KNOWN_PREDICTION) {
					msg2 = String.format(
							"\tSiNGL does not do any prediction for the above pairs as SiNGL is still collecting training data to build the preliminary models.");
				} else {
					msg2 = String.format("\tSiNGL predicts the above records %s with a similarity score of %.2f",
